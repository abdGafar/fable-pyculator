...
twenty-eight native Excel tables. Most tables begin on row 28 and use row 23 as a column role/source
marker band with values such as `AUX`, `DIRECT`, `SCEN`, `CALC`, `DATA-1`, `DATA-2`, `DATA-3`, and
`DATA-4`.
...
- The 2019 workbook has an older layout: `SCENARIOS definition` uses row 32 as the main role/source
  marker band and has twenty-three native tables.

Initial Phase 4 boundary:

- Discover table-level structures, not individual editable widgets.
- Preserve headers, row labels, cell refs, table refs, and per-column role/source markers.
